...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("select distinct p from Post p left join fetch p.images where p.user.id = :userId")
    List<Post> findPostWithImageByUserId(@Param("userId") Long userId);

    // in 쿼리를 이용해 followingIds 리스트에 있는 id들을 모아 한번에 조회
    @Query("select distinct p from Post p left join fetch p.images where p.user.id in :followingIds")
